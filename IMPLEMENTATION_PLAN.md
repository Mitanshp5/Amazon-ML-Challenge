...
```python
# US make_search_text (Proven Run 1 formula)
def make_search_text_us(name, addr):
    # Balanced name + first 3 address tokens (prevents national franchise chain collisions)
    addr_toks = addr.split()[:3]
    return f"{name} {' '.join(addr_toks)}".strip()
